...
A single Python script (`~/.ai-agent/scripts/sync_agent_rules.py`) with three subcommands (`init`, `sync`, `reconfigure`) that manages a canonical rule/skill store at `~/.ai-agent/` and generates agent-native configs.
...
| Interactive prompts | `input()` | Numbered multi-select via stdin. No TUI library needed. |
...
```
sync_agent_rules.py (~700 lines, single file)
├── Constants & paths
...
└── CLI entrypoints (cmd_init, cmd_sync, cmd_reconfigure)
```
...
3. Skip known system directories (`.system/`, `pattern-*`, migration artifacts)
